...
| # | Tarefa | DoD | Verificação |
|---|---|---|---|
| F1.0 | **ADR-0003 — licença e formato da CIQUAL**: ~~verificar na fonte oficial~~ ✅ 2026-08-15: 8 ficheiros baixados do Dataverse (DOI 10.57745/RDMHWY), MD5 = oficiais, etalab-2.0/SPDX confirmada; **XML passa a formato primário** (proveniência por valor; XLS sem fontes — doc oficial §3.1.1); semântica `-`/`traces`/`<N`/confiança A–D verificada (doc §3.2/§5.5); AMENDADO ADR-0001 A6. **Pendente: revisão humana do ADR** | ADR proposto; registry coincide | `nutridb sources audit` (pinned=yes) |
| F1.0b | **Versão fixada: CIQUAL 2025** ✅ 2026-08-15: SHA-256 dos 8 ficheiros fixados no registry (modelo `files` novo no registry.py); `sources sync` re-descarcou `alim_grp` do zero e validou por hash | Hash no registry | `uv run nutridb sources sync` ✓ |
| F1.1 | **Vocabulário canónico completo e congelado** (autoridade: INFOODS, complementado por cobertura CIQUAL 2025 e EuroFIR): `vocab/nutrients.csv` (~250 tagnames: energia ×2 com método, proximados, hidratos, lípidos + ácidos gordos `FnnDnnNn`, minerais, vitaminas, aminoácidos, outros), `nutrient_groups.csv`, `units.csv` (1 unidade imutável por nutriente), `value_types.csv`, `acquisition_types.csv`, `analytical_methods.csv` (lista inicial), `food_groups.csv` (taxonomia própria), `facets/` (7 ficheiros); relações de agregação (`nutrient_relation`) | Parse limpo; invariantes: tagname único, unidade única, grupos existem; commit de congelamento | `uv run nutridb vocab check` — 0 erros |
