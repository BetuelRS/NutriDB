...
| # | Tarefa | DoD | Verificação |
|---|---|---|---|
| F1.0 | **ADR-0003 — licença e formato da CIQUAL**: verificar na fonte oficial (URL, licença exata e versão, atribuição exigida, formato, volume, cobertura, células especiais `-`/`traces`/`<N`, unidades por coluna, granularidade de proveniência), concluir artefacto de destino (`core`) e registar no registry | ADR aprovado; registry coincide com o ADR | leitura do ADR; `nutridb sources audit` |
| F1.0b | **Versão fixada: CIQUAL 2025** (3 484 alimentos, 74 constituintes, etalab 2.0) — aprovado em revisão (A4/A5); fixar hash SHA-256 no registry após sync | Hash no registry | `nutridb sources sync` relê e valida |
