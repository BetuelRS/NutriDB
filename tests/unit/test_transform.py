"""Unit tests for the canonical transform (F1.4, SPEC §8, ADR-0001 D5).

Runs the F1.2 extractor over the synthetic fixture and asserts the
canonical dataset: typed values, unit conversion, absence semantics
(coverage + explicit flags, never a Cartesian product), provenance and
determinism.
"""
...
def _run(base: Path) -> dict[str, int]:
    extract(_make_cache(base), base / "i")
...
def _by_pair(rows: pl.DataFrame) -> dict[tuple[int, int], dict[str, object]]:
    return {
...
def test_value_provenance_full_chain(tmp_path: Path) -> None:
    ...
    assert food_json["alim_nom_fr"] == "Pastis"
